...
\emph{GraphQL} è, come suggeriscono le ultime due lettere del nome, un \emph{query language}, creato come supporto alla realizzazione di applicazioni client, che mette a disposizione una sintassi intuitiva e flessibile ed un sistema che permette al client di specificare i requisiti e le interazioni sui dati \cite{website:graphql-specs}. Rispetto all'approccio REST, GraphQL permette di effettuare richieste più efficienti dei dati ed evita la duplicazione della logica lato server che può verificarsi nel caso di endpoint personalizzati. Permette infatti di specificare quali informazioni servono per una vista e permette di effettuare diverse query tramite una singola richiesta.

\textcolor{red}{fare esempio di una query in graphql\\
	mettere in risalto la possibilità di richiedere quali dati si vogliono ricevere e perchè è utile\\
	mostrare paginazione\\
	fare esempio di interazione con più "tabelle" e confronto con caso REST che richiede più interazioni\\
	elencare principi}
