...
Si prenda come esempio un dispositivo mobile fornito ai visitatori di un museo di storia naturale che sfrutti il contesto per \emph{a)} adattare la struttura e l'aspetto dell'interfaccia grafica in base alle caratteristiche del visitatore, sia esso una persona con problemi di vista oppure un bambino; \emph{b)} mostrando informazioni diverse in base agli interessi del visitatore (es.: geologia, paleontologia, ecc.) oppure in base alla stanza in cui si trova; \emph{c)} imparare dalle precedenti scelte effettuate dall'utente su quali saranno le informazioni di maggiore interesse in futuro; \emph{d)} proponga al visitatore determinati servizi, come l'acquisto dei biglietti per una mostra temporanea o la prenotazione di un posto nello spettacolo sui dinosauri che inizia nella sala adiacente; \emph{e)} conosca la posizione del visitatore in base ai sensori che monitorano l'ambiente; \emph{f)} fornisca dei contenuti attivi al visitatore sul cosa accade in un particolare ambiente.

I sistemi \emph{context aware} riguardano l'\emph{acquisizione} del contesto (per esempio tramite l'utilizzo di alcuni sensori), l'\emph{astrazione} e il \emph{riconoscimento} del contesto (per esempio associare un determinato stimolo al contesto) ed il comportamento adattato al contesto riconosciuto (per esempio l'esecuzione di un'azione innescata dal contesto) \cite{schmidt2003ubiquitous}. L'utilizzo del \emph{contesto} permette la realizzazione di interfacce grafiche innovative e viene spesso utilizzato nella \emph{computazione ubiqua} e nei \emph{dispositivi indossabili}.
...
I mashup si possono distinguere in diverse tipologie, a prescindere dalla piattaforma sulla quale sono utilizzati.
In questo caso, la classificazione è basata soprattutto al modo in cui vengono integrate le diverse risorse che compongono i mashup. A partire da questo si ottengono tre macro famiglie:
\begin{enumerate}
	\item \textbf{Consumer Mashups} Sono i mashup che combinano elementi visuali e dati da diverse fonti. Un ottimo esempio può essere considerato Housing Maps citato in \ref{sec:mashup}, dove vengono integrati i dati provenienti da Craigslist e mostrati sulla mappa creata usando le API di Google Maps
	\item \textbf{Data Mashups} Sono mashup che combinano più risorse di dati in una singola. Esistono molti esempi di questa tipologia e sono relativi a diverse applicazioni.
	Nell'ambito di viaggi, un esempio può essere considerato Kayak \footnote{Kayak : \url{http://www.kayak.com}}, che è un sito di ricerca per viaggi che ottiene dati da oltre 100 siti diversi. Inoltre non vende direttamente i contenuti ai fornitori, ma si propone come un portale attraverso il quale i clienti possono essere dirottati verso le agenzie viaggi per servire le loro necessità.
	Un altro ambito completamente distinto può essere considerato il caso di Facile.it \footnote{Facile.it : \url{http://www.facile.it}}, che permette di confrontare i dati provenienti da diverse offerte per quanto riguarda una grande quantità di dati (es.: assicurazioni, mutui, linee telefoniche, ecc.) permettendo di visualizzarli in unico portale e valutare la proposta migliore per le esigenze dell'utente
	\item \textbf{Business Mashups} I business mashups sono utili per quanto riguarda l'integrazione di servizi business e dati, permettendo di sviluppare più facilmente servizi integrati e, di conseguenza, semplificare l'utilizzo all'utente attraverso interfacce web. Si differiscono dai \emph{consumer mashups} a livello di integrazione con i sistemi business, perché sono necessarie funzionalità aggiuntive, riguardanti sicurezza, controllo di accesso, governance e complessità degli editor utilizzati. Un'altra differenza è data dall'utilizzo crescente di business mashups nei modelli software as a service (Saas), dove un produttore sviluppa e gestisce un'applicazione web che mette a disposizione dei propri clienti via internet, come servizio cloud.
\end{enumerate}
...
Le informazioni grezze ottenute direttamente dai servizi esterni non è detto che siano già pronte per essere visualizzate all'utente finale. Esse necessitano di elaborazioni per poter migliorare e integrare i diversi dati ricevuti. Come esposto in \cite{caio2011tesi} queste operazioni possono essere fatte mediante template visuali, i quali mostrano una rappresentazione dei dati come saranno mostrati nell'applicazione finale. Nel caso di applicazioni sui contenuti si hanno generalmente due schermate: una mostra tutti i risultati provenienti dai diversi servizi, l'altra invece mostra i dettagli dell'elemento selezionato. Le operazioni sui dati da svolgere sono due:
\begin{enumerate}
	\item \textbf{Union} L'operazione di union mostra tutti gli elementi che provengono dai diversi servizi, cercando di trovare le similitudini e quindi aggregare le risposte che si riferiscono allo stesso elemento
	\item \textbf{Merge} L'operazione di merge permette di mostrare tutti i dettagli dell'istanza selezionata. In questo modo i dati provenienti da più servizi sono mostrati nella stessa schermata. Questa operazione è definita durante la composizione del mashup, quando vengono associati i termini al servizio corrispondente.
\end{enumerate}
