...
Questo capitolo illustra la realizzazione del backend dell piattaforma CAMUS. Il capitolo inizia con l'analisi dello schema dell'architettura e la descrizione delle tecnologie che sono state utilizzate. In seguito vengono presentati due descrittori che definiscono il comportamento dell'applicazione nelle diverse situazioni di utilizzo, che sono quello dell'albero di contesto e quello dei servizi. Viene mostrato prima il modello concettuale del database, con una descrizione delle entità e relazioni che lo compongono, e successivamente lo schema logico risultante. Una volta definita la base di dati, si passa all'analisi dettagliata dei principali componenti che fanno parte del sistema, con l'ausilio di diagrammi e pseudocodice per una maggiore comprensione. Viene quindi mostrato come dall'esterno è possibile eseguire le attività che il backend mette a disposizione tramite l'unico endpoint fornito da GraphQL. Per una maggiore chiarezza viene mostrato il flusso che una richiesta percorre nel backend affinché venga evasa, mostrando nel dettaglio le varianti che possono verificarsi. In conclusione viene mostrato come è possibile configurare alcuni parametri del sistema per modificarne il comportamento, attraverso variabili d'ambiente e file di configurazione.
...
Altre entità vengono utilizzate per la memorizzazione delle informazioni sull'u\-ten\-te. Un'altra entità importante riguarda i \emph{mashup}, che sono associati all'utente per permettere la definizione di schemi personalizzati. I dettagli verranno esposti nella Sezione \ref{sec:struttura-schemi-mashup}.
...
\begin{figure}[p]
	\centering
	\includegraphics[width=\textheight, angle=90]{5-implementazione-backend/Immagini/diagramma_classi_backend.png}
...
\textcolor{red}{Definire meglio il funzionamento delle ricerche ed espandere, senza ripetere cose già dette nelle metodologie}
...
\textcolor{red}{rimuovere cose già dette}
...
Il resto di questa sezione descriverà in dettaglio le varie attività che possono essere invocate. Esse saranno identificate per semplicità di espressione col nome di endpoint, visto che il fine ultimo è paragonabile a quello degli endpoint nel mondo REST.
