...
\lstinputlisting[style=blocco-esempi,
	caption=Esempio di nodo del CDT,
	label=lst:esempio-nodo-cdt]{5-implementazione-backend/Codice/esempio_nodo_cdt.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-parametro-cdt]{5-implementazione-backend/Codice/esempio_parametro_cdt.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-descrittore-servizio]{5-implementazione-backend/Codice/esempio_descrittore_servizio.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-descrittore-operazione]{5-implementazione-backend/Codice/esempio_descrittore_operazione.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-descrittore-parametro]{5-implementazione-backend/Codice/esempio_descrittore_parametro.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-formato-risposta]{5-implementazione-backend/Codice/esempio_formato_risposta.json}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:esempio-descrittore-paginazione]{5-implementazione-backend/Codice/esempio_descrittore_paginazione.json}
...
\begin{itemize}
	...
	\item \textbf{Node Parameter} Questa entità rappresenta un parametro associato ad un nodo del CDT. Ad un parametro possono essere associati diversi \virgolette{campi}, nel caso in cui un parametro sia definito da diversi sottoattributi. Per esempio, il parametro \emph{Località} può essere specializzato nei campi \emph{Latitudine} e \emph{Longitudine}
...
\lstinputlisting[caption=Esempio richiesta executeQuery,
	label=lst:esempio-richiesta-execute-query]{5-implementazione-backend/Codice/esempio_richiesta_execute_query.graphql}

\subsection{Login\label{sec:login-endpoint}}

Questo endpoint è dedicato all'autenticazione degli utenti. Riceve in ingresso l'indirizzo \emph{mail} e la \emph{password}, cifrata con algoritmo SHA1\footnote{SHA1: \url{https://en.wikipedia.org/wiki/SHA-1}}, digitati dall'utente. Se i due parametri corrispondono ad un utente vengono restituiti un \emph{token}, utilizzato per mantenere aperta la sessione, ed il \emph{nome} e \emph{cognome} dell'utente, per personalizzare l'app con le informazioni personali.
...
\lstinputlisting[caption=Esempio richiesta login,
	label=lst:esempio-richiesta-login]{5-implementazione-backend/Codice/esempio_richiesta_login.graphql}
...
\lstinputlisting[caption=Esempio richiesta getPersonalData,
	label=lst:esempio-get-personal-data]{5-implementazione-backend/Codice/esempio_get_personal_data.graphql}
...
\lstinputlisting[style=blocco-esempi,
	...
	label=lst:file-configurazione-predefinito]{5-implementazione-backend/Codice/file_configurazione_predefinito.json}