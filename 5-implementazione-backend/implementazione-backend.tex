...
Introduzione ... \textcolor{red}{completare}

Per interfacciarsi con le web app e l'app mobile, sono stati realizzati diversi endpoint, che verranno descritti nel dettaglio nelle seguenti sottosezioni.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
	\item \textbf{Support} Contiene l'elenco delle categoria di servizi di supporto che si vogliono ricevere
\end{itemize}
...
Questo endpoint provvede a fornire all'utente i suoi dati personali, che possono essere gli \emph{alberi di contesto} e i \emph{mashup} che gli sono stati associati dall'esperto di settore. In questo modo l'app mobile può recuperare le versioni più aggiornate degli schemi relativi all'utente. Come input vengono richiesti l'identificativo dell'\emph{utente} e il \emph{token} ricevuto nella fase di autenticazione. Se i due valori corrispondono, restituisce i dati richiesti. Il formato della risposta viene composto come segue:
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item \textbf{Enum} Elenco dei possibili valori che il parametro può assumere
...
Nelle successive sezioni verranno analizzate nel dettaglio ognuna delle precedenti fasi.
...
Questa attività incomincia una volta creato il \emph{CDT decorato} ed opera in parallelo all'acquisizione dei servizi di supporto. Coinvolge unicamente il componente \emph{Support Service Selection}, che si occupa di selezionare i servizi di supporto o gli intent richiesti dal client. In ingresso viene fornita la \emph{categoria} dei servizi che si vuole selezionare in base al contesto fornito.
...
Per configurare agilmente determinati parametri del sistema, vengono sfruttate determinate \emph{variabili d'ambiente} e viene messa a disposizione una cartella dove aggiungere dei \emph{file di configurazione}. In caso di conflitto viene seguita la seguente scala di priorità:
...
\begin{itemize}
	\item \textbf{NODE\_ENV} Per definire l'ambiente nel quale avviare l'applicazione (es.: production, development, ecc.)
...
Nella cartella \virgolette{config} è possibile aggiungere dei file per configurare altri parametri. Possono essere creati più file di configurazione, che verranno caricati per i diversi ambienti nel quale può funzionare l'applicazione. Basta chiamare il file col nome dell'ambiente di destinazione (es.: production, development, qa, staging, ecc.). Viene fornita la configurazione \virgolette{default} che viene utilizzata quando non è presente nessun file relativo all'ambiente corrente. Di seguito vengono elencati i parametri accettati. Tra parentesi sono indicati i valori predefiniti di ciascun campo.
