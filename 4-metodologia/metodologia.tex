...
Un altro aspetto fondamentale del sistema è quello dell'utilizzo dei mashup per costruire le view dell'applicazione mobile e per associare a ogni singolo elemento il termine da richiedere al server. Questo viene eseguito dall'esperto di settore per mezzo della web app di Visual Mapping (Figura \ref{fig:visual-mapping}), la quale genera come output uno schema che comprende i mashup occorrenti per visualizzare i dati che servono all'utente finale.

\upe stato scelto di adottare due tipologie di schemi fondamentali, uno per la lista di risultati e uno per i dettagli dell'elemento selezionato, perché fondamentalmente sono queste due le pagine dell'applicazione che necessitano di associazioni dei termini con gli elementi grafici.
Gli schemi generati possono essere ulteriormente differenziati per due motivi principali. 

\begin{enumerate}
	\item Il primo di essi è la tipologia dei dati, che nel nostro caso viene differenziata usando il parametro dell' \virgolette{Interest Topic}.
	...
	\item
	Il secondo aspetto è la possibilità di differenziare i dati da mostrare a livello di utente, dato che non a tutti possono interessare gli stessi dati oppure può essere preferibile mostrare i dati e le view in maniera differente in stili grafici e componenti. Infatti, una volta generato lo schema, viene associato nel database del server all'id dell'utente, in maniera tale da renderli disponibili su richiesta della mobile app, dopo che è stato effettuato il login.
	Nella web app l'esperto seleziona l'utente al quale deve generare l'applicazione e, in aggiunta alle associazioni al CDT, carica gli schemi generati sul server, i quali vengono associati all'id dell'utente. In questo modo ogni utente dell'applicazione può avere dei mashup personalizzati.
	In aggiunta alla definizione dei termini nei mashup, è possibile aggiungere le proprietà stilistiche relative al singolo schema, se diverse da quelle definite globalmente.
\end{enumerate}

\begin{figure}[ht]
	\centering
	\includegraphics{4-metodologia/Immagini/view_schema.png}
	\caption{Schema delle views}\label{fig:view-schema}
\end{figure}
Lo schema delle view che è stato adottato è raffigurato nella figura \ref{fig:view-schema}. A ogni pagina definita viene associato uno o più valori di \virgolette{Interest Topics}, perché può essere che uno schema venga utilizzato per più di un topic. Ogni pagina può avere più di un contenuto al quale è associato opzionalmente uno stile e la tipologia del contenuto, che serve poi all'app a renderizzare il componente associato.
...
\begin{itemize}
	...
	\end{enumerate}
	Ad esempio per mostrare il titolo dell'elemento selezionato l'esperto deve prendere un campo di testo dai componenti, trascinarlo all'interno del mashup dell'applicazione e associare il campo titolo dai termini sulla sinistra della web app. A questo è possibile aggiungere degli elementi di stile aggiuntivi, che, aggiunti allo schema, sovrascrivono eventuali stili presenti di default nell'app.
...
\begin{itemize}
	\item \textbf{Server centric} In questa tipologia di architettura le operazioni vengono eseguite sul server, precisamente, dato un mashup, si occupa di tutte le operazioni di richiesta, integrazione e composizione dei dati, portando tutta la complessità delle operazioni sul server.
	\item \textbf{Client centric} Tutte le richieste verso i servizi esterni vengono eseguite viene scaricato sul dispositivo, quindi le operazioni di richiesta, integrazione e composizione vengono eseguite direttamente dal dispositivo.
\end{itemize}
...
Data la crescente potenza di calcolo dei processori destinati al comparto mobile la tentazione di far fare molte operazioni anche complesse al dispositivo mobile è molto forte, come presentato nella precedente implementazione del sistema \cite{rizzo2015progettazione}. In quella versione del sistema nello schema era specificato il metodo per poter effettuare le query direttamente verso i servizi primari e i risultati venivano elaborati dal dispositivo mobile, che quindi si occupava di tutte le operazioni sui dati (\emph{Union} e \emph{Merge}).
Una delle prime osservazioni che sono state fatte è legata ai tempi di risposta dei servizi esterni, dovuto sia ai server dei provider che alle condizioni di rete.  Per questo motivo non è detto che due servizi abbiano i medesimi tempi di risposta, quindi l'applicazione potrebbe ottenere delle risposte parziali o comunque avere tempi di risposta più alti nel caso di una richiesta senza caching. Questo inoltre aggiunge un problema con le API keys dei servizi, che, nel caso di tante richieste verso il medesimo servizio da più client, porterebbe a un esaurimento dei limiti o a maggiori costi per il servizio dati. Un chiaro esempio di servizi di questo tipo può essere considerato Google Places\footnote{Google Places Developers: \url{https://developers.google.com/places/}}, che offre un numero limitato di queries giornaliere verso il servizio usando una sola API key.
Esiste un altro problema, il quale è legato all'autonomia e all'utilizzo dei dati del mobile device. L'esecuzione di richieste e operazioni sui dati porta ovviamente a un maggiore consumo di risorse hardware di calcolo e di telecomunicazione, utilizzando maggiormente la batteria integrata e, anche se i dati scambiati non sono di grandi dimensioni.
Questi problemi verrebbero risolti utilizzando un unico punto di esecuzione delle query con un meccanismo di caching, in modo da evitare ripetizioni di richieste, tentando di ridurre il numero di query verso i servizi esterni. Inoltre, anche la mobile app non deve consumare maggiori risorse richiedendo dati da più servizi, ma utilizza un unico punto dal quale riceve dei dati già pronti per essere visualizzati e già filtrati dal server.
