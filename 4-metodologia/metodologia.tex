...
Un esempio viene rappresentato in Figura \ref{fig:associazione-servizi-cdt} ... \textcolor{red}{da completare}
...
\begin{center}
	\begin{align*}
		...
		&RestaurantType : FishCuisine\ \land \\
		&Transport : PublicTransport\ \land \\
		&Location\ (Latitude : ``45.478897"",\ Longitude : ``9.2342429")
	\end{align*}
	\captionof{figure}{Esempio di contesto\label{fig:esempio-contesto}}
\end{center}
...
\subsection*{Ricerca tramite località}

Come definito nella Sezione \ref{sec:associazione-servizi-cdt}, è possibile effettuare anche ricerche nei dintorni di una località. Come mostrato anche in Figura \ref{fig:esempio-contesto}, nel contesto possono essere definite le coordinate geografiche del luogo nel quale ci si trova. Queste informazioni hanno un duplice scopo: \emph{i)} selezionare le operazioni che hanno validità locale; \emph{ii)} essere utilizzate come parametro per invocare i servizi. In questa sezione ci si focalizza sulla selezione delle operazioni mentre per dettagli sul come vengono composte le query verso i servizi si rimanda alla Sezione \ref{sec:bridge}.

In questo caso, le coordinate vengono utilizzate per ricercare quali operazioni sono state definite in un intorno variabile, definito a livello di sistema, rispetto alla posizione corrente. L'elenco delle operazioni risultanti viene ordinato in base alla distanza tra la posizione alla quale è stata associata l'operazione e la posizione corrente. Per quanto riguarda le \emph{operazioni primarie}, questo ordine viene sfruttato per assegnare il valore di \emph{priorità} alle operazioni, in quanto varia in base alla posizione effettiva nella quale viene effettuata l'interrogazione.

Vista la rilevanza delle operazioni che hanno validità locale, il nodo corrispondente alla \emph{località} è di tipo \emph{ranking}, in modo che le associazioni ricavate da questa fase abbiamo un peso maggiore e che possano scalare la classifica più agevolmente. La motivazione principale risiede nel fatto che se un'operazione è stata assegnata ad una località specifica significa che è un servizio specializzato e quindi ci si aspetta dei risultati più accurati rispetto ad un altro più generale.
